...
Training Maximum Entropy document classifiers using Generalized Expectation Criteria is described in this separate `tutorial <http://mallet.cs.umass.edu/ge-classification.php>`_.

To get started with classification, first load your data into MALLET format as described in `the importing <import.html>`_ data section.
...
Currently supported algorithms include MaxEnt, NaiveBayes, C45, DecisionTree and many others. See the `JavaDoc API <http://mallet.cs.umass.edu/api>`_ for the cc.mallet.classify package to see the complete current list of Trainer classes.
...
`bin/mallet train-classifier --input labeled.mallet --training-portion 0.9  --report train:accuracy test:f1:sports`
